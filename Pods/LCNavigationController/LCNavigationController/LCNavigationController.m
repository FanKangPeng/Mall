...
static const CGFloat LCAnimationDuration = 0.40f;   // Push / Pop 动画持续时间
static const CGFloat LCMaxBlackMaskAlpha = 0.80f;   // 黑色背景透明度
static const CGFloat LCZoomRatio         = 0.9;   // 后面视图缩放比
...
- (void)gestureRecognizerDidPan:(UIPanGestureRecognizer*)panGesture {
  ...
  //  CGPoint touchPoint = [panGesture locationInView:self.view];
    ...
    if (panGesture.state == UIGestureRecognizerStateEnded || panGesture.state == UIGestureRecognizerStateCancelled) {
        ...
        if (fabs(vel.x) > [UIScreen mainScreen].bounds.size.width/2) {
            
            [self completeSlidingAnimationWithDirection:panDirection Offset:offset];
...
- (void)completeSlidingAnimationWithDirection:(PanDirection)direction  Offset:(CGFloat)offset  {
