...
+ (instancetype)picScrollViewWithFrame:(CGRect)frame WithImageUrls:(NSArray<NSString *> *)imageUrl {
    return  [[FKPScrollerView alloc] initWithFrame:frame WithImageNames:imageUrl];
}
...
- (UIPageControl *)PageControl
{
    if (!_PageControl) {
        ...
        _PageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0,myHeight - pageSize,myWidth, 7)];
...
- (UIImage *)setImageWithIndex:(NSInteger)index {
  UIImage *image =   [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:_imageData[index]];
    if (image == nil) {
        
        UIImageView * imageview =[UIImageView new];
        [imageview sd_setImageWithURL:[NSURL URLWithString:_imageData[index]] placeholderImage:[UIImage imageNamed:@"image_loadding_background.jpg"]];
        [[SDImageCache sharedImageCache] storeImage:image forKey:_imageData[index]];
        //从内存缓存中取,如果没有使用占位图片
