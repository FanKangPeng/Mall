...
#define STARTPOINT CGPointMake(kScreenWidth-30, 200)
...
- (id)initWithFrame:(CGRect)frame menus:(NSArray *)aMenusArray andView:(UIView *)subView
{
    ...
    if (self) {
        ...
        for (int i = 0; i < count; i ++)
        {
            ...
            item.tag = 1000 + i;
            item.startPoint = CGPointMake(kScreenWidth-30, 200);
           
            if(i==0)
            {
                item.endPoint = CGPointMake(180,140);
                item.nearPoint = CGPointMake(240,170);
                item.farPoint =CGPointMake(160, 120);
                item.center = item.startPoint;
            }
            
            if(i==1)
            {
                item.endPoint = CGPointMake(237,150);
                item.nearPoint = CGPointMake(260,175);
                item.farPoint =CGPointMake(217, 130);
                item.center = item.startPoint;
            }
            if(i==2)
            {
                item.endPoint = CGPointMake(300,170);
                item.nearPoint = CGPointMake(300,180);
                item.farPoint =CGPointMake(300, 150);
                item.center = item.startPoint;
            }
            
            NSLog(@"endpoint.x=%f----%f,,,farPoint-----%f----%f,,,,,,nearPoint-----%f-----%f,,,,,startPoint---%f----%f",item.endPoint.x,item.endPoint.y,item.farPoint.x,item.farPoint.y,item.nearPoint.x,item.nearPoint.y,item.startPoint.x,item.startPoint.y);
        ...
        // add the "Add" Button.
        _addButton = [[QuadCurveMenuItem alloc] initWithImage:[UIImage imageNamed:@"community_award"]
                                       highlightedImage:nil
                                           ContentImage:nil
                                highlightedContentImage:nil];
        _addButton.delegate = self;
        _addButton.center = CGPointMake(self.frame.size.width-30, self.frame.size.height-10);
...
- (void)_close
{
    ...
    self.frame =CGRectMake(kScreenWidth-60, 13, 51, 20);
    int tag = 1000 + _flag;
     QuadCurveMenuItem *item = (QuadCurveMenuItem *)[self viewWithTag:tag];
    
    CAKeyframeAnimation *rotateAnimation = [CAKeyframeAnimation animationWithKeyPath:@"transform.rotation.z"];
    rotateAnimation.values = [NSArray arrayWithObjects:[NSNumber numberWithFloat:0.0f],[NSNumber numberWithFloat:0.0f],[NSNumber numberWithFloat:0], nil];
