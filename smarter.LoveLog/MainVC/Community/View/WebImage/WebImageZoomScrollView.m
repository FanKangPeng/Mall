...
- (void)calculateImageFrame:(UIImage *)image{
    ...
    CGFloat imageWidth  = image ? image.size.width :kScreenWidth;
    CGFloat imageHeight = image ? image.size.height : kScreenHeight/2;
    float scaleX = self.scrollView.width/imageWidth;
    float scaleY = self.scrollView.height/imageHeight;
    
    if (scaleX > scaleY)
    {
        float imgViewWidth =image ? image.size.width * scaleY :kScreenWidth * scaleY;
        
        self.imageView.frame = CGRectMake((self.scrollView.width - imgViewWidth) * 0.5,0 ,imgViewWidth, self.scrollView.height);
    }else{
        float imgViewHeight =image ? image.size.height*scaleX : kScreenHeight/2 *scaleX;
